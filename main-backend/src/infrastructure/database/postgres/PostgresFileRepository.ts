...
export class PostgresFileRepository implements IFileRepository {
  private static readonly updatableColumns: (keyof Omit<File, 'id' | 'created_at' | 'updated_at' | 'user_id'>)[] = [
        'document_id',
        'source_type',
        'file_name',
        'source_location',
  ...
  async update(file: Partial<File> & { id: string }): Promise<File | null> {
    ...
    const validFields = Object.keys(fieldsToUpdate)
            .filter(key => 
    ...
    const values = validFields.map(key => fieldsToUpdate[key as keyof typeof fieldsToUpdate]);
